...
int main() {
    ...
    // IHDR
    ASSERT_TRUE(p + 8 <= n);
    uint32_t ihdr_len = be32(buf + p);
    p += 4;
    ASSERT_EQ_I(ihdr_len, 13);
    ASSERT_TRUE(memcmp(buf + p, "IHDR", 4) == 0);
    p += 4;
    ASSERT_TRUE(p + ihdr_len + 4 <= n);
    const unsigned char* ihdr = buf + p;
    p += ihdr_len;
    uint32_t ihdr_crc = be32(buf + p);
    p += 4;
    ...
    ASSERT_TRUE(p + 8 <= n);
    uint32_t idat_len = be32(buf + p);
    p += 4;
    ASSERT_TRUE(memcmp(buf + p, "IDAT", 4) == 0);
    p += 4;
    ASSERT_TRUE(p + idat_len + 4 <= n);
    const unsigned char* idat = buf + p;
    p += idat_len;
    uint32_t idat_crc = be32(buf + p);
    p += 4;

    // Verify IDAT CRC
    unsigned char* tmp = (unsigned char*)malloc(4 + idat_len);
    memcpy(tmp, "IDAT", 4);
    memcpy(tmp + 4, idat, idat_len);
    ...
    // IEND
    ASSERT_TRUE(p + 8 <= n);
    uint32_t iend_len = be32(buf + p);
    p += 4;
    ASSERT_EQ_I(iend_len, 0);
    ASSERT_TRUE(memcmp(buf + p, "IEND", 4) == 0);
    p += 4;
    // CRC of "IEND" is fixed (0xAE426082)
    ASSERT_EQ_U32(be32(buf + p), 0xAE426082u);
    p += 4;
